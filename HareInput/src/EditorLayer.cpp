...
namespace Hare
{
	...
	{
		std::optional<std::string> filepath = FileDialogs::OpenFile("Hare Scene (*.hare)\0*.hare\0");
		if (filepath)
		{
			m_ActiveScene = CreateRef<Scene>();
			m_ActiveScene->OnViewportResize((uint32_t)m_ViewportSize.x, (uint32_t)m_ViewportSize.y);
	...
	{
		std::optional<std::string> filepath = FileDialogs::SaveFile("Hare Scene (*.hare)\0*.hare\0");
		if (filepath)
		{
			SceneSerializer serializer(m_ActiveScene);
